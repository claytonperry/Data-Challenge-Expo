#Scenario 3: Alternate Scenario 2, all states implement at the same time
...
restrictions_as2 <- restrictions %>%
  select(state, date, mon, yr, C1_as2:C8_as2) %>%
  rename(C1=C1_as2,C2=C2_as2,C3=C3_as2,C4=C4_as2,C5=C5_as2,C6=C6_as2,C7=C7_as2,C8=C8_as2)

analytic2_0 <- st_pop %>%
  inner_join(confdaily, by = 'state') %>%
  inner_join(restrictions_as2, by = c('state','date')) %>%
  group_by(state) %>%
  mutate(d = row_number()) %>%
  ungroup() %>%
...
analytic2_1 <- schedule %>%
  ...
  select(Imsi, sex, state, yearmonth, newcases_hat, PWEIGHT, weight, EEDUC, raceth, agebin, C1_as2_sum1:C8_as2_sum1) %>%
...
for (i in unique(analytic2_1$state)) {
  glm_list[[i]] <- glm(Imsi ~ newcases_hat + sex + agebin + raceth + EEDUC +
                         C1_as2_sum1 + C2_as2_sum1 + C3_as2_sum1 + C4_as2_sum1 +
                         C5_as2_sum1 + C6_as2_sum1 + C7_as2_sum1 + C8_as2_sum1, subset = state == i, data = analytic2_1, family = binomial)
  predictions_list[[i]] <- predict(glm_list[[i]], type = 'response')
  attributes(predictions_list[[i]]) <- NULL
...
analytic2_2 <- schedule %>% #start with schedule 
  ...
  select(CLF, SEX, EEDUC, agebin, raceth, WTFINL, newcases_hat, yearmonth, state,
         C1_as2_sum1, C2_as2_sum1, C3_as2_sum1, C4_as2_sum1, C5_as2_sum1, C6_as2_sum1, C7_as2_sum1, C8_as2_sum1) %>% #retain relevant variables
...
for (i in unique(analytic2_2$state)) {
  glm_list[[i]] <- glm(CLF ~ newcases_hat + SEX + agebin + raceth + EEDUC +
                         C1_as2_sum1 + C2_as2_sum1 + C3_as2_sum1 + C4_as2_sum1 +
                         C5_as2_sum1 + C6_as2_sum1 + C7_as2_sum1 + C8_as2_sum1, subset = state == i, data = analytic2_2, family = binomial)
  predictions_list[[i]] <- predict(glm_list[[i]], type = 'response')
  attributes(predictions_list[[i]]) <- NULL
