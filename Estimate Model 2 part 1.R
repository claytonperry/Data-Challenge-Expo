...
analytic2_1 <- st_pop %>%
  ...
  mutate(sinfd = 0.5 * (sin((d/20.69) - 1.82) + 1),
         ...
         cds = newcases_d/pop,
         across(C1:C8,list(i1 = l1_ind, i2 = l2_ind, i3 = l3_ind))) %>%
  group_by(state) %>%
  mutate(C1_min1 = min(d[which(C1==1)]),
            ...
            C8_min1 = min(d[which(C8_i1==1)]),
            C8_min2 = min(d[which(C8==2)]),
            C8_min3 = min(d[which(C8==3)])) %>%
  ungroup() %>%
  mutate(across(contains('C'), function(x) replace(x, is.infinite(x), NA))) %>%
  mutate(C1_fade1 = 1 / (1 + exp(d - C1_min1 - 8)),
